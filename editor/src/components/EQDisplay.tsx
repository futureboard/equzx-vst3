  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (engine && L.analyzerMode !== 'off') {
        const pre = L.analyzerMode !== 'post' ? engine.getSpectrum('pre') : null
        if (pre) {
          peaks = drawSpectrum(
            ctx, pre, L.x, L.inner,
            {
        ...
        const post = L.analyzerMode !== 'pre' ? engine.getSpectrum('post') : null
        if (post) {
          drawSpectrum(
            ctx, post, L.x, L.inner,
            {
...
/** Draws one analyser trace, tilted so pink noise reads flat. Returns decayed peaks. */
function drawSpectrum(
  ...
  const { db: source, fMin, fMax, floorDb } = spectrum
  ...
  // Both the curve and the axis are logarithmic, so a pixel column maps to a
  // fractional index by a plain ratio — no bin arithmetic, no aggregation. The
  // curve arrives already reduced, from the Rust analyser or from Web Audio.
  const span = Math.log(fMax / fMin)
  for (let i = 0; i < cols; i++) {
    const fLo = x.invert(i)
    const fHi = x.invert(i + 1)
    const fMid = Math.sqrt(fLo * fHi)
    const pos = clamp((Math.log(fMid / fMin) / span) * (points - 1), 0, points - 1)
    const db = sampleBins(source, pos, floorDb)
  ...
  const dbSpan = spectrum.ceilDb - spectrum.floorDb
  const toPy = (db: number) => inner.h - clamp((db - spectrum.floorDb) / dbSpan, 0, 1.15) * inner.h
