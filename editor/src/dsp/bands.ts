...
/**
 * A band with a caller-chosen id.
 *
 * Only the plugin build uses this. There, a band's id *is* its parameter slot in
 * the plugin — that is what keeps a host automation lane pointed at the band the
 * user drew — so the id has to come from the outside rather than a counter.
 */
export function makeBandInSlot(slot: number, partial: Partial<Band> = {}): Band {
  return { ...makeBand(partial), id: slot }
}

/** Lowest slot no band is occupying, or null when the bank is full. */
export function freeSlot(bands: Band[], maxBands = MAX_BANDS): number | null {
  const taken = new Set(bands.map((b) => b.id))
  for (let slot = 0; slot < maxBands; slot++) {
